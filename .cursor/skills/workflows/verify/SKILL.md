...
6. **Work on one issue at a time.** For each reported bug or gap: draft a **bug-fix plan** (and design tests before the fix). **Architect review (bug-fix plan):** Run the **architect review gate** (see Notes) on that plan (one review set per bug; max two passes), incorporate feedback, surface any new ADRs, then review the proposed fix with the user before implementing it.
7. After the user agrees the fix plan: implement tests, then the fix. When the fix is done, draft a short **post-fix narrative** (what changed, caveats, tests). **Architect review (post-fix narrative):** Run the architect review gate on that narrative (separate review set; max two passes), incorporate feedback, surface any new ADRs, then present it to the user before committing.
