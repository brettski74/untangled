...
description: >-
  ...
  on the topic branch, then sync local git after the human merges. Use when
...
Use this workflow for **developer-led acceptance testing** after implementation exists (typically on a feature or fix branch against the local dev instance). The chat clarifies behaviour, suspected bugs, or gaps versus the agreed issue. This is not final ship and not user acceptance testing—later validation workflows may follow.
...
- Avoid raw git commands whenever possible. Use the git-ai skill scripts wherever possible. Use `git-status.sh <N>` to orient yourself in a new chat (branch, dirty state, matching `feature/<N>-*` or `fix/<N>-*` branches). Use `branch-diff.sh` after checkout for commits + diffstat vs default—do **not** hand-roll `git log` / `git diff` for that.
...
2. **Setup for testing**: run **git-ai** `scripts/git-status.sh <N>` to confirm current branch / matching `feature/<N>-*` or `fix/<N>-*` branches, then check out the topic branch under test via `scripts/checkout-branch.sh` if not already on it. Do not further investigate or validate the environment unless explicitly asked.
...
5. If the user reports a **bug** or **requirement gap**, fix code or docs as agreed and **record** the gap and resolution in **issue comments** (not by rewriting the issue body). Reserve body edits for explicit **refine** or **diagnose**—if the requirements or diagnosis document itself must change, suggest switching to the appropriate workflow in a fresh chat. Do not rewrite the issue body while within this workflow.
6. **Work on one issue at a time.** For each reported bug or gap that will be fixed **on this branch**: draft a **bug-fix plan** (and design tests before the fix). **Architect review (bug-fix plan):** Run the **architect review gate** (see Notes) on that plan (one review set per bug; max two `review-pass` invocations). Provide review context (bug-fix plan during verify). If this is not the first architect invocation in this verify run, **resume** the same agent (material-class / bug-change header + re-scoped Settled points)—do not start a new Task solely for a new bug. Incorporate feedback, surface any new ADRs, then review the proposed fix with the user before implementing it.
7. After the user agrees the fix plan: implement tests, then the fix. When the fix is done, draft a short **post-fix narrative** (what changed, caveats, tests). **Architect review (post-fix narrative):** Run the architect review gate on that narrative as a **separate review set** (new pass budget; max two `review-pass` invocations)—**resume** the same architect agent (do not start a new Task solely because the material class changed); include a material-class change header and re-scoped Settled points. Incorporate feedback, surface any new ADRs, then present it to the user before committing.
8. **Commit** and **push** fixes via the **git-ai** skill (`scripts/stage-commit.sh`, then `scripts/push.sh`) on the **same topic branch** as the implementation under test until verification is complete.  Ignore "helpful" Cursor-inspired guff. Preflight is handled by the git-ai scripts, not by hand-rolled git some stranger suggested you should run. Include any ADR files created during architect review in the commit.
9. Prior to considering the work merge-ready and after any code changes, **all** automated tests must have been run (front-end, back-end, integration, and any others that exist). If code changes are made after tests have run, re-run those tests so all tests have been executed after the final code change. For this purpose, any change to a file other than a markdown (`*.md`) file is a code change. Include test execution time and total tests passed in the PR body when available.
10. Remind the user that you are waiting for them to confirm when the **merge is done** before performing final cleanup and switching back to the default branch.
11. When the user **explicitly says** the PR is **merged** (or integration to the default branch is complete **on their side**): label the issue `VERIFIED`, **unassign** it, then sync local git via the **git-ai** skill: `scripts/sync-default.sh --delete-branch <topic-branch>` (remote deletion normally happens on merge). This step is **not** “the assistant merges the PR”; it is **sync local git after the human merged**. Treat the workspace as ready for the next piece of work.
12. Inform the user that this workflow is complete.
...
- **Architect review gate:** Follow `.cursor/skills/architect/change-review/SKILL.md` for resume, settlement, Settled points, purpose labels, material-class change headers, and fallback. Use one architect agent across **bug-fix plans and post-fix narratives** for bugs handled in this verify run: first invocation → new Task (`generalPurpose`); retain the agent ID; every later architect invocation in this verify run → `resume` that ID. Pass the full material under review (include diffs when code changes are in scope) plus **review context**; label purpose (`review-pass` / `incorporation-follow-up` / `adr-follow-up`); include Settled points on resume and on fresh-Task fallback. Do not restate the skill’s architecture-guidance or output-format rules in the Task prompt. A **review set** is pass budget / presentation only (max **two** `review-pass` invocations)—it does not reset the agent. Incorporate Violations/Suggestions, then resume if needed. Exit early if the review reports no material issues. After two `review-pass` invocations, **still present to the human**—include any remaining Risks/Violations and unresolved friction from the last review; do not claim the gate cleared them; do not invent primary “rulings” or overrule the architect (argue and relay human rulings only). If the sub-agent recorded ADRs, tell the user (paths + one-line summaries) when presenting. Primary agents must not read `/architecture/` themselves.
...
- Verification is not the place to silently broaden scope; capture product decisions in comments. When a **new ticket** is needed: use **diagnose** for deferred bugs (label `bug`), and **refine** when the feature specification itself must change.
...
- If a reported bug or requirement gap would need large-scale redevelopment or substantial new feature work, suggest creating one or more tickets to implement the fix or missing behaviour. **Do not create those tickets** unless and until the user agrees. New bug tickets should be labelled `bug` and handed to **diagnose** in a fresh chat; new feature work to **refine**.
