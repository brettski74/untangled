...
9. When the user **explicitly says** the PR is **merged** (or integration to the default branch is complete **on their side**): label the issue `VERIFIED`, **unassign** it, then sync local git via the **git-ai** skill: `scripts/sync-default.sh --delete-branch <feature-branch>` (remote deletion normally happens on merge). This step is **not** “the assistant merges the PR”; it is **sync local git after the human merged**. Treat the workspace as ready for the next piece of work.
