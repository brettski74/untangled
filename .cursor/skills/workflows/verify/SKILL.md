...
6. **Work on one issue at a time.** For each reported bug or gap: draft a **bug-fix plan** (and design tests before the fix). **Architect review (bug-fix plan):** Run the **architect review gate** (see Notes) on that plan (one review set per bug; max two `review-pass` invocations). If this is not the first architect invocation in this verify run, **resume** the same agent (material-class / bug-change header + re-scoped Settled points)—do not start a new Task solely for a new bug. Incorporate feedback, surface any new ADRs, then review the proposed fix with the user before implementing it.
...
11. When the user **explicitly says** the PR is **merged** (or integration to the default branch is complete **on their side**): label the issue `VERIFIED`, **unassign** it, then sync local git via the **git-ai** skill: `scripts/sync-default.sh --delete-branch <feature-branch>` (remote deletion normally happens on merge). This step is **not** “the assistant merges the PR”; it is **sync local git after the human merged**. Treat the workspace as ready for the next piece of work.
