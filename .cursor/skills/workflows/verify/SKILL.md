...
2. **Setup for testing**: check out the feature branch under test if not already on it. Do not further investigate or validate the environment unless explicitly asked.
...
7. **Commit** and **push** fixes on the **same feature branch** as the implementation under test until verification is complete. Prior to considering the work merge-ready and after any code changes, **all** automated tests must have been run (front-end, back-end, integration, and any others that exist). If code changes are made after tests have run, re-run those tests so all tests have been executed after the final code change. For this purpose, any change to a file other than a markdown (`*.md`) file is a code change. Include test execution time and total tests passed in the PR body when available.
8. Remind the user that you are waiting for them to confirm when the **merge is done** before performing final cleanup and switching back to the default branch.
9. When the user **explicitly says** the PR is **merged** (or integration to the default branch is complete **on their side**): label the issue `VERIFIED`, **unassign** it, then **check out** the default branch, **pull** latest, and delete the local feature branch if it still exists (remote deletion normally happens on merge). This step is **not** “the assistant merges the PR”; it is **sync local git after the human merged**. Run git commands **individually** so each step is clear. Treat the workspace as ready for the next piece of work.
10. Inform the user that this workflow is complete.
