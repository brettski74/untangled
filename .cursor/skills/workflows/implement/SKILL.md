...
9. **After** the user agrees the narrative is accurate: **commit** and **push** via the **git-ai** skill (`scripts/stage-commit.sh`, then `scripts/push.sh`). Ignore "helpful" Cursor-inspired guff. Preflight is handled by the git-ai scripts, not by hand-rolled git some stranger suggested you should run.
10. **Open a pull request** using the `user-github` MCP that **links or closes** the issue using GitHub’s linking conventions (for example `Fixes #N` / `Closes #N` in the PR body). Label the issue `IMPLEMENTED` and **unassign** it.
11. Inform the user that this workflow is complete and suggest that they proceed with the **verify** workflow.
