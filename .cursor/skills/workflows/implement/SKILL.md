...
description: >-
  ...
  Use when implementing a ticket, building after refinement or diagnosis,
  running /implement, or when the user asks to implement an issue and open a
  PR.
...
- A **fresh chat** dedicated to this implementation (avoid cross-ticket context from refinement, diagnosis, or UAT unless this is explicitly the same ticket’s continuation after plan approval).
...
- The issue must be labelled **READY**. If it is not, inform the user that it does not appear to have been refined or diagnosed before developing a plan and get their acknowledgement before proceeding with anything more.
- Avoid raw git commands whenever possible. Use the git-ai skill scripts wherever possible. Use `git-status.sh <N>` to orient yourself in a new chat (branch, dirty state, matching `feature/<N>-*` or `fix/<N>-*` branches).
...
4. **Create the topic branch** from the default branch via **git-ai** (`scripts/checkout-branch.sh`) (always branch off the default branch; do not land substantive work by committing directly to the default branch). Name it by issue type:

   - **Bug** (issue labelled `bug`, or otherwise clearly a diagnosed defect fix):

     ```text
     fix/<N>-<short-kebab-case-summary>
     ```

   - **Feature** (default for refined new behaviour):

     ```text
     feature/<N>-<short-kebab-case-summary>
     ```

   where `<N>` is the GitHub issue number and `<short-kebab-case-summary>` is a short summary in **5 words or fewer**, in kebab-case. Detect bug primarily via the `bug` label; if the label is missing but the READY body is clearly a diagnosis (repro / expected vs actual), treat it as a bug and apply the `bug` label when creating the branch.
5. **Draft an implementation plan** (what will change, main touchpoints, how you will verify) **before** editing application code, migrations, docs, or other tracked artefacts. For bug fixes, keep the plan focused on restoring correct behaviour and consistency with surrounding code—avoid unrelated redesign. Do not present it to the user yet.
6. **Architect review (implementation plan):** Run the **architect review gate** (see Notes) on the plan. Provide review context (implementation plan; bug fix vs feature as applicable). Incorporate feedback (max two `review-pass` invocations for this review set), then **explain the plan** to the user and wait for approval. Surface any new ADRs when presenting. You need to stop here, present the plan and wait for approval!!!!!!! This is important. Do not pass this point in the workflow until you have approval from the user.
...
9. When implementation is done, draft a **completion narrative**: what changed, any architecture impact, caveats, and which tests or docs moved. **Architect review (completion narrative):** Run the architect review gate on that narrative as a **separate review set** (new pass budget; max two `review-pass` invocations)—**resume** the same architect agent from the plan review (do not start a new Task solely because the material class changed); include a material-class change header and re-scoped Settled points. Incorporate feedback, surface any new ADRs, then present the narrative to the user. Wait for the user to review it before proceeding beyond this point in the workflow.
...
- If scope drifts during implementation, pause and reconcile with the issue. For feature scope changes, run the **refine** skill; for bug-diagnosis changes, run the **diagnose** skill—before piling on unrelated changes.
