...
5. **Architect review (requirements draft):** Before pointing the human at the draft for substantive review, run the **architect review gate** (see Notes) on `.refinement/<N>-draft.md`. Incorporate feedback (max two architect passes), then surface any new ADRs to the user when presenting the draft.
...
- **Architect review gate:** Launch `change-review` in a **separate** Task (`generalPurpose`) per `.cursor/skills/architect/change-review/SKILL.md`. Pass the full draft under review in the Task prompt. Refine never has code changes in scope—do not include diffs. Do not restate the skill’s architecture-guidance or output-format rules in the Task prompt. Max **two** architect passes per review set: incorporate Violations/Suggestions into the draft, then re-invoke if needed. Exit early if the review reports no material issues. After two passes, **still present to the human**—include any remaining Risks/Violations from the last review; do not claim the gate cleared them. If the sub-agent recorded ADRs, tell the user (paths + one-line summaries) when presenting. Primary agents must not read `/architecture/` themselves.
