...
| `frontend/app/records/` | SSR search/fetch/update seams (Bearer via session). Fetch/search/update use `/api/v1` with FK identity enrichment on responses. Shared helpers in `fk_identity.ts`. |
